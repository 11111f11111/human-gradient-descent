...
项目中的文件结构是 GPT 的工作区，不是要求用户填写的表格。用户也不需要背诵完整指令：只要消息中出现“炼丹炉”，GPT 就先识别自然语言意图，再在后台调用完整内部协议。
...
GPT 应先反馈“识别任务、内部调用、关键约束、是否需要确认”，随后执行部署、资料识别和初始化。旧格式 `炼丹炉｜部署`、`炼丹炉｜初始化` 仍然兼容。
