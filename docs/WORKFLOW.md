...
Human Gradient Descent 将课程资料、学生作答、错因、掌握度和会话断点保存在 GitHub 中。AI 每次开始学习前读取这些状态，结束后再将新证据写回。因此，聊天窗口可以结束，但学习状态不会随窗口一起消失。

它不是让模型获得永久记忆，而是给不同窗口和不同模型提供一个共同、可检查的外部状态仓库。
...
```mermaid
flowchart TD
    A["课程资料层<br/>课件、习题、答案、历年题"] --> B["AI 教学层<br/>讲解、提问、测试、诊断"]
    C["学习状态层<br/>掌握度、错题、学习特征、会话断点"] --> B
...
1. 使用本仓库创建一个新课程项目。
2. 填写 `config/course.yaml`：课程名称、考试形式、日期、允许材料和题型。
3. 将资料放入 `sources/`：
   - `slides/`：课件；
   - `exercises/`：课程习题；
   - `solutions/`：官方答案；
   - `past_exams/`：历年试题。
4. 在 `sources/manifest.yaml` 登记文件来源、范围和可信程度。
5. 在 `status/mastery.yaml` 建立章节清单，初始状态设为 `NOT_STARTED`。
6. 在 `status/learner_profile.yaml` 填写用户已经明确表达的学习偏好；未知项保持空白，禁止 AI 猜测。
7. 要求 AI 阅读 `AGENTS.md`，检查项目结构，再生成第一阶段学习计划。

## 5. 每次新窗口怎么开始

可以直接发送：

> 请按照 AGENTS.md 的读取顺序恢复学习状态。先告诉我上次完成了什么、哪些判断有测试证据、当前最需要处理什么，然后从断点继续。
...
不能只读取 `mastery.yaml`。掌握状态可能滞后，必须与近期测试、错题和会话交接相互核对。
...
仓库不会自动把内容注入所有 AI。新窗口必须能够访问仓库，并遵守 `AGENTS.md` 的读取顺序；否则仍可能表现得像第一次见到用户。
...
学生上传课件、习题和历年题。AI 没有立即开始泛泛总结，而是：
...
新窗口先读取仓库，因此不需要学生重新解释课程结构。学习过程中，学生能复述概念，但在题目变化后无法独立确定解题入口。
...
这些信息没有依赖某个窗口的临时记忆，而是由仓库中的学习证据、状态文件和交接记录共同恢复。
