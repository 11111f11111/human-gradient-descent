...
- 最终输出：Agent 提出压缩建议，用户确认范围后生成可检索资料。

## 4. 第一次使用

项目中的文件结构是 GPT 的工作区，不是要求用户填写的表格。用户也不需要背诵完整指令：只要消息中出现“炼丹炉”，GPT 就先判断用户是在调用系统还是讨论系统。只有调用任务时才进入完整内部协议。
...
我已经上传课程资料，目标是完成学习与考试准备。
```

简单任务只需一行路由说明；复杂、复合或有风险的任务才反馈“识别任务、内部调用、关键约束、是否需要确认”。旧格式 `炼丹炉｜部署`、`炼丹炉｜初始化` 仍然兼容。
...
AI 在后台先读取核心规则和断点，再按任务加载需要的证据：

1. `AGENTS.md`：工作和证据规则；
2. `commands/spellbook.yaml`：路由与完整协议；
3. `config/course.yaml`：课程与考试约束；
4. `handoff/latest.md`：上次会话的断点；
5. 当前任务需要的掌握度、错题、知识库、习题、测试、考试或原始资料。
...
4. 理解确认后进行不附答案提示的测试。
5. 高 Loss 分别标记为 `HUMAN`、`MATERIAL`、`BOTH` 或 `UNCERTAIN`，再细分人的梯度和资料的梯度。
6. 开卷和闭卷证据分别更新；只有在声明的条件下独立完成并具备迁移证据，才标记为 `PASS`。
7. Agent 判断内容是否值得进入开卷资料，但先写入待办；只有用户明确确认后才修改正式材料。
...
Gemini 等其他 AI 理论上也可以采用“仓库链接 + 课程资料 + AI 管理文件系统”的方法，但必须针对其具体能力重新实现。本仓库只保证 GPT + GitHub 参考流程经过实际验证。

不建议由两个 AI 同时维护同一个项目。它们可能采用不同的掌握标准、文件结构理解和更新时机，导致重复记录或状态互相覆盖。需要更换 AI 时，应先说“炼丹炉，结束本次学习并写好交接”完成收炉，再让新的 AI 读取全部状态并成为唯一 active writer。
...
| 尚待确认的资料改进 | `output/openbook/update_queue.md` | 汇总资料梯度，但不静默改变正式讲义 |
...
AI 据此安排复习优先级，并把值得压缩的内容写入 `output/openbook/update_queue.md`：题型识别信号、必要公式、固定步骤、易混概念和常见错误。用户确认本次范围后，AI 才修改正式开卷资料；长篇推导保留在知识库，通过目录链接快速定位。
