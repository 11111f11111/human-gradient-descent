...
Human Gradient Descent 将课程资料、学生作答、错因、掌握度和会话断点保存在一个可持续读写的项目文件夹中。它可以位于云盘、GitHub、本地同步目录或其他持久化文件空间。AI 每次开始学习前读取状态，结束后再将新证据写回。因此，聊天窗口可以结束，但学习状态不会随窗口一起消失。

它不是让模型获得永久记忆，而是给不同窗口和不同模型提供一个共同、可检查的外部状态空间。模型可以更换，只要新模型拥有同一目录的读写权限并遵守本项目协议。
...
项目中的文件结构是 AI 的工作区，不是要求用户填写的表格。用户通过指令完成部署：
...
项目文件不会自动注入所有 AI。新窗口必须能够访问同一持久化目录，并遵守 `AGENTS.md` 和 `COMMANDS.md`；否则仍可能表现得像第一次见到用户。
