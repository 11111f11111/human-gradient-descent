...
本项目采用双层指令架构：用户只用自然语言说明目标，AI 在后台把它转换为完整、严格的内部执行协议。**只要用户消息中出现“炼丹炉”三个连续汉字，AI 就必须先判断是在调用系统还是讨论系统。**只有调用任务时才执行协议；讨论名称或规则不会误写学习状态。
...
简单任务只返回一行路由说明；复杂、复合或有风险的任务才说明识别结果、内部协议、关键约束和需要确认的问题。判断明确且操作可撤销时，说明后直接执行。
...
以下步骤由 AI 自动执行，不是用户操作说明。运行时先读取核心规则和断点，再按任务选择相关证据，避免每个简单任务都加载全部文件：

1. `AGENTS.md`
2. `commands/spellbook.yaml`
3. `config/course.yaml`
4. `handoff/latest.md`
5. 当前任务需要的 `status/`、`mistakes/`、`sources/`、`tests/`、`exam/`、`kb/`、`exercise/` 或 `output/`
...
- `PASS`：在明确声明的开卷或闭卷条件下，能独立识别题型、完成核心步骤并检查结果。
...
Agent 会根据新官方资料、资料梯度、来源冲突和已验证内容判断是否值得修改，但默认只写入 `output/openbook/update_queue.md`。只有用户明确要求或确认具体待办后，才能修改正式开卷资料。上传新文件本身不等于授权，也不保证一定需要修改。
