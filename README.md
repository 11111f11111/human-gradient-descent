...
这是一个可部署到云盘、GitHub 或其他持久化文件空间的空白课程模板。它不是普通笔记目录，而是一套让不同 AI 与学生持续协作的学习状态系统。

## 1. 怎么使用

用户不需要理解或手动维护本仓库中的 YAML、Markdown 和目录结构。这些文件是 AI 使用的学习状态后端。

任何能够持续读取并写回同一项目文件夹的 AI 都可以运行本系统。第一次使用只需要发送部署指令：
...
普通使用者无需操作这些目录；本节供维护模板或审查 AI 写入结果时参考。
...
系统不绑定具体 AI 或网盘服务。可使用的组合包括任何具备以下能力的模型与存储：

- 能读取项目中的 Markdown、YAML 和课程资料；
- 能在任务结束后写回或更新文件；
- 能在新窗口重新访问同一项目目录；
- 能遵守 `AGENTS.md` 和 [`COMMANDS.md`](COMMANDS.md)。

如果某个 AI 只能读取网盘、不能写回，它可以临时讲解，但不能独立完成跨窗口状态更新。

完整指令格式与可复制示例见 [`COMMANDS.md`](COMMANDS.md)。
